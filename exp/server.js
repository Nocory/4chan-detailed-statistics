...
expressApp.get('/analyzeText', (req, res) => {
	const word = req.query.word || "ERROR"

	pino.debug(`expressApp.get /analyzeText for word ${req.query.word}`)
...
expressApp.get('/analyzeTokens', (req, res) => {
	const word = req.query.word || "ERROR"

	pino.debug(`expressApp.get /analyzeTokens for word ${req.query.word}`)
	pino.debug(`wordPromiseQueue: ${wordPromiseQueue.length} - ${wordPromiseQueue}`)
	...
	const newPromise = new Promise(async (resolve,reject) => {
		try{
			await prevPromise
			const result = await analyzeText.analyzeTokens(req.query.word)
			res.send(result)
		}catch(err){
			pino.error(err.message)
			res.status(500).send()
...
expressApp.get('/findMentions', async (req, res) => {
	pino.debug(`expressApp.get /findMentions for word ${req.query.word}`)
	try{
		res.send(await analyzeText.findMentions(req.query.word))
	}catch(err){
